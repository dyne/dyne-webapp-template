...
(defn reload-session [request]
  ...
  (conf/load-config "{{name}}" conf/default-settings)
...
(defn render-head
  ([] (render-head
       "{{name}}" ;; default title
       "{{name}}"
       "https://{{name}}.dyne.org")) ;; default desc
  ...
  ([title desc url]
   [:head [:meta {:charset "utf-8"}]
    ...
    (page/include-css "/static/css/{{name}}.css")]))
   ...
   {:class "navbar navbar-default navbar-fixed-top navbar-expand-md navbar-expand-lg"}
    [:div {:class "navbar-header"}
     [:button {:class "navbar-toggle" :type "button"
      ...
      [:span {:class "icon-bar"}]
      [:span {:class "icon-bar"}]]
     [:a {:class "navbar-brand far fa-handshake" :href "/"} "{{name}}"]]
...
(defonce login-form
  [:div
   [:h1 "Login for your {{toaster}} account"
    [:form {:action "/login"
            :method "post"}
