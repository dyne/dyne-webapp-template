...
;;                      :ns-whitelist  ["agiladmin.*" "just-auth.*"]
  ...
  (reset! config (conf/load-config
                  (or (System/getenv "{{name}}_conf") "{{name}}")
                  conf/default-settings))

  (let [justauth-conf (get-in @config [:{{name}} :just-auth])]
    ...
    (f/attempt-all
     [auth-conf   (get-in @config [:{{name}} :just-auth])
     ...
     [(trans/init "lang/auth-en.yml" "lang/english.yaml")
      ...
      (reset! auth (auth/email-based-authentication  
                    ...
                    (dissoc (:just-auth (:{{name}} (conf/load-config
                                                    "{{name}}" conf/default-settings)))
